...
allom_lookup <- allom_lookup %>%
  add_row(genus = "aegiceras", species = "corniculatum", sps_code = "aeco", density = NA,
          ag_form = c(NA), bg_form = c(NA), ag_ref = "ren2010", bg_ref = NA) %>%
  add_row(genus = "avicennia", species = "alba", sps_code = "aval", density = 0.506, 
          ag_form = c(komi2005ag), bg_form = c(komi2005bg), ag_ref = "komiyama2005", bg_ref = "komiyama2005") %>%
  add_row(genus = "avicennia", species = "marina", sps_code = "avma", density = NA,
          ag_form = c(function(dbh, dens){0.1848*(dbh^2.3524)}), bg_form = NA, ag_ref = "clough1997", bg_ref = NA) %>%
  ...
  add_row(genus = "bruguiera", species = "parviflora", sps_code = "brpa", density = NA,
          ag_form = c(function(dbh, dens){10^(-0.7045+2.5336*(log10(dbh)))}), bg_form = c(NA), ag_ref = NA) %>%
  ...
  add_row(genus = "excoecaria", species = "agallocha", sps_code = "exag", density = NA,
          ag_form = c(function(dbh, dens){exp(1)^(1.0996*(log(dbh^2))-0.8572)}), bg_form = NA, ag_ref = "hossain2015", bg_ref = NA) %>%
  add_row(genus = "kandelia", species = "obovata", sps_code = "kaob", density = NA,
          ag_form = c(function(dbh, dens){0.251*0.525*(dbh^2.46)}), bg_form = NA, ag_ref = "khan????", bg_ref = NA) %>%
  add_row(genus = "lumnitzera", species = "racemosa", sps_code = "lura", density = NA,
          ag_form = c(function(dbh, dens){1.184+(DBH.cm[k])^2.384}), bg_form = NA, ag_ref = "kangkuso2015", bg_ref = NA) %>%
  add_row(genus = "rhizophora", species = "apiculata", sps_code = "rhap", density = NA,
          ag_form = c(function(dbh, dens){exp(1)^((2.318*log(dbh)) - 1.671)}), bg_form = NA, ag_ref = "ong2004", bg_ref = NA) %>%
  ...
  add_row(genus = "rhizophora", species = "stylosa", sps_code = "rhst", density = NA,
          ag_form = c(function(dbh, dens){10^(-0.6564+2.4292*(log10(dbh)))}), bg_form = NA, ag_ref = "clough1997", bg_ref = NA) %>%
